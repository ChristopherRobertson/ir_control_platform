"""Server-rendered UI primitives for the Phase 3A shell."""
...
.state-box.unavailable { background: #eef5f7; border-color: #8eb8c7; }
.device-card, .session-card, .timeline-step, .readiness-row, .event-row {
  padding: 10px 0;
  border-top: 1px solid #ecebe4;
}
.device-card:first-child, .session-card:first-child, .timeline-step:first-child,
.readiness-row:first-child, .event-row:first-child { border-top: none; padding-top: 0; }
.status-label {
...
def render_run_page(page: RunPageModel, scenario_id: str) -> str:
    ...
    <div class="panel-actions">
      <form method="post" action="/run/start">
        <input type="hidden" name="scenario" value="{escape(scenario_id)}">
        <button type="submit">Start Nominal Run</button>
      </form>
    ...
    live_data = "".join(render_live_data_series(series) for series in page.live_data) or (
        '<div class="small">No live data has been produced for this scenario yet.</div>'
    )
    ...
    <section class="section-grid">
      ...
      <div class="panel">
        <h3>Live Data</h3>
        <p class="panel-subtitle">Simulator-backed live data shell for the golden path.</p>
        {live_data}
      </div>
    </section>
    <section class="panel">
...
def render_results_page(page: ResultsPageModel, scenario_id: str) -> str:
    ...
    details = "".join(f"<li>{escape(detail)}</li>" for detail in page.manifest_details)
    manifest = f"<ul>{details}</ul>" if details else (
        '<div class="small">Select a session to inspect the manifest scaffolding.</div>'
    )
    return f"""
    <section class="panel">
      <h2>{escape(page.title)}</h2>
...
def render_service_page(page: ServicePageModel) -> str:
    ...
    <section class="section-grid">
      ...
      <div class="panel">
        <h3>Phase 3A Limits</h3>
        <p class="panel-subtitle">Service is scaffold-only in this phase.</p>
        <ul>{notes}</ul>
      </div>
    </section>"""
...
def render_live_data_series(series: LiveDataSeries) -> str:
    rows = "".join(
        "<tr>"
        f"<td>{point.wavenumber_cm1:.1f}</td>"
        f"<td>{point.value:.3f}</td>"
        "</tr>"
        for point in series.points
    )
    return (
        f'<div class="panel-subtitle">{escape(series.label)} ({escape(series.units)})</div>'
        '<table class="data-table"><thead><tr><th>Wavenumber (cm^-1)</th><th>Value</th></tr></thead><tbody>'
        + rows
        + "</tbody></table>"
    )
...
def render_run_step(step: RunStepSummary) -> str:
    ...
    <div class="timeline-step">
      ...
      <div>{escape(step.summary)}</div>
    </div>"""


def render_session_card(card: SessionSummaryCard, scenario_id: str) -> str:
    tone = "good" if card.status_label.lower() == "completed" else "warn"
    return f"""
    <div class="session-card">
      ...
      <div class="small">
        Raw {card.raw_artifact_count} | Processed {card.processed_artifact_count} |
        Analysis {card.analysis_artifact_count} | Exports {card.export_artifact_count}
      </div>
      <div class="panel-actions">
